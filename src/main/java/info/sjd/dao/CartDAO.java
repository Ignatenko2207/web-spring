...
public class CartDAO {
	...
	public static List<Cart> getCartsByTime(long from, long till) {
		...
		result = session.createQuery("from carts c where c.time between :from and :till").setParameter("from", from)
	...
	public static List<Cart> getCartsByUserId(int id) {
		...
		result = session.createQuery("from carts c where c.userId = :id").setParameter("id", id).getResultList();
	...
	public static Cart getCartByUserIdAndOpen(int id) {
		...
		resultCart = (Cart) session.createQuery("from carts c where c.userId = :id and c.status = :status").setParameter("id", id).setParameter("status", Status.OPEN).uniqueResult();
